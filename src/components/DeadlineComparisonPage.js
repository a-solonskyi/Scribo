...
export default function DeadlineComparisonPage() {
  ...
  return <main className="deadline-comparison">
    ...
    <section className="comparison-heading"><div><h1>One gesture. A continuous flow.</h1><p>Paper cut bottle + Fine line glass</p></div><button type="button" className="comparison-replay" onClick={replay}>Replay both</button></section>
    <div className="comparison-panes">
      {[{ id: "hybrid", label: "Before", note: "Previous motion" }, { id: "liquid", label: "Refined", note: "Liquid text · Curved pour · Rim spray" }].map((item) => <section key={item.id} className="comparison-pane" aria-label={item.label}>
        <div className="comparison-pane-heading"><h2>{item.label}</h2><span>{item.note}</span></div>
        <div className="deadline-comparison-stage">
          <AssignmentDeadline key={`${item.id}-${run}`} variant={item.id} deadline={deadline} startedAt={start} contained initiallyOpen={run > 0}
